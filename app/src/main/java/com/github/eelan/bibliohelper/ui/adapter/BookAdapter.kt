...
class BookAdapter(val items: BookList, val click: (Book) -> Unit) : RecyclerView.Adapter<BookAdapter.ViewHolder>() {
    ...
    fun getThumbUrlFromID(idBook: String, Taille: Int) = "http://books.google.be/books/content?id=$idBook&printsec=frontcover&img=1&zoom=$Taille&edge=curl&source=gbs_api"
}